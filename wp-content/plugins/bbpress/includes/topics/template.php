...
function bbp_topic_admin_links( $args = array() ) {
	...
	function bbp_get_topic_admin_links( $args = array() ) {
		...
		if ( empty( $r['links'] ) ) {
			...
			$author_id = (int) bbp_get_topic_author_id($topic->ID);
			...
			if( strcmp($author_role, 'administrator') && (get_current_user_id()!=$author_id) ){
			    $custom_array['spam'] = bbp_get_topic_spam_link ( $r );
			}
			

			$r['links'] = apply_filters( 'bbp_topic_admin_links', $custom_array, $r['id'] );
