	...
	function bbp_get_reply_admin_links( $args = array() ) {
		...
		if ( empty( $r['links'] ) ) {
			...
			*/
			...
			$author_id = (int) bbp_get_reply_author_id($reply->ID);
			$author_data = get_userdata($author_id);
			$author_roles = $author_data->roles;
			$author_role = $author_roles[0];

			// not admin, author != current_user
			if( strcmp($author_role, 'administrator') && (get_current_user_id()!=$author_id) ){
				$custom_array['spam'] = bbp_get_reply_spam_link ( $r );
			}

			$r['links'] = apply_filters( 'bbp_reply_admin_links', $custom_array, $r['id'] );
