jQuery(document).ready(function($) {
	...
	$('#kc-pro-list-els input[name="kc_pro_all"]').on('click', function(e){
		$('#kc-pro-list-els input[type="radio"]').attr({ checked : false });
		$('#kc-pro-list-els input[value="'+this.value+'"]').attr({ checked : true });
	});
	...
	$('#kc-pro-settings-process-download').on('click',function(){
		...
		$(this).off('click').attr({disabled:true}).removeClass('button-primary').html('<i class="fa-spinner fa-spin fa-fw"></i> KC Pro! is now installing...');
		...
		$.post( ajaxurl, {
				...
				if( result === -1 || result === 0 ){
					$('#kc-pro-settings-process-download').html('<span style="color:maroon"><i class="fa-times"></i> Installation failed</span>, reasons bellow:');
					$('#kc-pro-settings-download-wrp').append('<div class="kc-download-failed-mesg"><strong>Console:</strong><br />Invalid sercurity sessition or do wrong way.</div>');
				}else if( result.toString().indexOf( 'active-success' ) > -1 ){
					$('#kc-pro-settings-process-download').html('<span style="color:green"><i class="fa-check"></i> Installation successful</span>, reloading...');
					window.location.href = window.location.href.toString().split('#')[0];
				}else{
					$('#kc-pro-settings-process-download').html('<span style="color:maroon"><i class="fa-times"></i> Installation failed</span>, reasons bellow:');
					$('#kc-pro-settings-download-wrp').append('<div class="kc-download-failed-mesg">'+result+'</div>');
					$('#kc-pro-settings-download-wrp .kc-download-failed-mesg a').remove();
				}
			}
		);
    ...
    $('#kc-settings-verify-btn').on('click', function(){
		...
		$(this).attr({disabled:true}).removeClass('button-primary').html('<i class="fa-spinner fa-spin fa-fw"></i> verifying your license...');
		...
		jQuery.post(
			...
			function (result) 
			{
				...
				$('#kc-settings-verify-btn').attr({disabled:false}).addClass('button-primary').html('Verify your license');
