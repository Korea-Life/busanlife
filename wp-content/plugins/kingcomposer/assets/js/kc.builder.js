			...
			callbacks : {
				...
				background : function( wrp, $, data ){
					...
					var test = wrp.find('.kc-css-background-test').get(0), 
						inputs = wrp.find('input[data-css-background]'), p,
						values = {};
						...
						values = $.extend( { color: 'transparent', linearGradient: ['',''], image: 'none', position: '0% 0%', size: 'auto', repeat: 'repeat', attachment: 'scroll' }, values );
						
					var process_bgimg = function( str ){
						
						var gradient, image, colors = ['', ''];
						
						str = str.split('url');
						
				        gradient = str[0].match(/gradient(\(.*\))/g);
				        
				        if( gradient !== null && gradient.length ){
					        
				            gradient = gradient[0].replace(/(linear|radial|from|\bto\b|gradient|top|left|bottom|right|\d*%)/g,'');
				            
				            if( gradient.indexOf('rgba') > -1 ){
					            colors= $.grep( gradient.match(/(rgba\([^\)]+\)|#[a-z\d]*|[a-z]*)/g), function (s) {
						            return $.trim( s ) != '';
						        });
					        }else{
						        colors= $.grep( gradient.match(/(rgb\([^\)]+\)|#[a-z\d]*|[a-z]*)/g), function (s) {
						            return $.trim( s ) != '';
						        });
						    }
				        }
						...
						if( str[1] == undefined )
							image = '';
						else image = str[1].replace('(','').replace(')','').replace(/\"/g,'').trim();
				         
				        return { colors: colors, image: image };
					...
					for( var f in values ){
						...
						if( f == 'color' && ( values[f] == 'rgba(0, 0, 0, 0)' || values[f] == 'transparent' ) ){
							...
							inputs.filter('[name="linearGradient"]').val( values[f][0] );
							inputs.filter('[name="linearGradient-bottom"]').val( values[f][1] );
						}else inputs.filter('[name="'+f+'"]').val( values[f] );
						
					}
					
					wrp.find('.field-color_picker input').each(function(){
					...
					inputs.on('change', wrp, function(e){
						
						// transparent none 0% 0%/auto repeat scroll
						var factory = { color: 'transparent', linearGradient: ['',''], image: 'none', position: '0% 0%', size: 'auto', repeat: 'repeat', attachment: 'scroll', advanced: 0 }, 
							...
							for( var f in factory ){
								...
								if( val !== null && val !== undefined && val !== '' && val != factory[f] ){
									if( f == 'linearGradient' ){
										...
										factory[f] = [val, inputs.filter('[name="'+f+'-bottom"]').val()];
						...
						e.data.find('input[data-css-background-value]').val( kc.tools.base64.encode(JSON.stringify( factory ) ) ).change();
				...
				radio_image : function( wrp ){
					...
					wrp.find('label.rbtn img').on('mouseover mousemove',{el: preview, wd: win_width} , function(e){
