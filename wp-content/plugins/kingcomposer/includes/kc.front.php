...
class kc_front{
							...
							else
							{
								...
								else if($sel[0] == 'border')
								{
									...
									if (is_array( $json ))
									{
										...
										{
											if( $css_obj['linearGradient'][1] === '' )
												$css_obj['linearGradient'][1] = $css_obj['linearGradient'][0];
												
											$val .= 'linear-gradient('.$css_obj['linearGradient'][0].','.$css_obj['linearGradient'][1].')';
		...
		if( is_array( $obj ) ){
			...
			foreach( $obj as $screen => $properties ){
				...
				if( $screen == 'any' ){
					...
					if( strpos( $screen, '-' ) === false ){
						$css .= '@media only screen and (max-width: '.trim($screen).'px){'.implode('', $properties).'}';
					}else{
						$screen = explode('-', $screen);
						$css .= '@media only screen and (min-width: '.trim($screen[0]).'px) and (max-width: '.trim($screen[1]).'px){'.implode('', $properties).'}';
					}
	...
	private function render_dynamic_css(){
		...
		if (!empty($this->css))		
			echo '<style type="text/css" id="kc-css-general">'.$this->css.'</style>';
