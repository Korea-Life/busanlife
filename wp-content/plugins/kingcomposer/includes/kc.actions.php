...
function kc_filter_admin_menu_title( $menu_title ){
    ...
    if ( isset( $current->response[ KC_BASE ] ) || ( defined( 'KCP_BASENAME' ) && isset( $current->response[ KCP_BASENAME ] ) ) )
		$menu_title .= '&nbsp;<span class="update-plugins"><span class="plugin-count">1</span></span>';
	...
	if( isset( $_POST['kc_post_meta'] ) && isset( $_POST['kc_post_meta']['mode'] ) && $_POST['kc_post_meta']['mode'] == 'kc' ){
		...
		*/
		$content_processed = str_replace( 
			array( "\n", '<style type="text/css">', 'body.kc-css-system' ), 
			array( "", '<style type="text/css">'.kc_basic_layout_css(), 'html body' ), 
			$content_processed 
		);
